...
public final class CompressCmd implements Callable<Integer> {
    ...
    @Option(names = { "-d",
            "--dictionary" }, description = "Custom dictionary location (JSON).", paramLabel = "<uri|file>")
    URI dictionary = null;

    @Option(names = { "-x",
            "--hex" }, description = "Output result as hexadecimal-encoded. Automatically enabled for stdout.")
    boolean hex = false;
    ...
    public Integer call() throws Exception {
        ...
        final var document = input.fetch();

        final var json = document.getJsonContent()
                .orElseThrow(() -> new IllegalArgumentException(
                        "Invalid input document. JSON document expected but got [" + document.getContentType() + "]."));

        if (JsonUtils.isNotObject(json)) {
            throw new IllegalArgumentException(
                    "The input docunent root is not JSON object but [" + json.getValueType() + "].");
        }

        final var encoder = CborLd.createEncoder(
                switch (mode) {
        ...
        if (output == null) {
            ...
            try (final var os = new FileOutputStream(output)) {
                if (hex) {
